...
random_label = (
    "Available on"  # Label for random picks in mix mode (default: "Random pick from")
)
...
def generate_background_for_item(
    ...
    if base_background is None or overlay is None or plex_logo is None:
        print(
            f"[ERROR] Background images were not provided to generate_background_for_item function"
        )
...
if __name__ == "__main__":
    ...
    try:
        base_background = Image.open(os.path.join(BASE_PATH, "bckg.png")).convert(
            "RGBA"
        )
